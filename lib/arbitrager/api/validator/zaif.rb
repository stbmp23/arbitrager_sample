    ...
    module Validator
      class Zaif
        ...
        def get_orders_response(params)
          result = response_success?(:get_orders, params)
          result && params.has_key?('return') && params['return'].is_a?(Hash)
        end
        ...
        # @params [Hash, Array] JSON.parse(response.body)
        # @return [true, false]
        def get_active_orders_response(params)
          response_success?(:get_active_orders, params)
        end
        ...
        def get_balance_response(params)
          response_success?(:get_balance, params)
        end
        ...
        def response_success?(method_name, params)
          if params['success'].blank? || params['success'] != RESPONSE_SUCCESS || !params.has_key?('return')
        ...
        # レスポンスチェックエラーを発生させる
        #
        # @params [Symbol] method_name メソッド名
        # @params [Hash, Array, nil] params JSON.parse(response.body)
        # @return [false]
        def error(method_name, params = nil)
          message = "Zaif.#{method_name} のレスポンスがおかしいです. レスポンス: #{params}"
