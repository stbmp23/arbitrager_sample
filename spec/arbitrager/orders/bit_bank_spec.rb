...
describe Arbitrager::Orders::BitBank do
  let(:order_ask) { Arbitrager::Orders::BitBank.create_ask(2_000_000.0, 0.001) }
  let(:order_bid) { Arbitrager::Orders::BitBank.create_bid(2_000_000.0, 0.001) }
      ...
      context "正常完了" do
        ...
        it "注文情報がモデルへ保存される" do
          ...
          expect(attributes).to eq({
            'broker_id' => Settings.broker.bitbank.id,
  ...
  describe "#execution?" do
    ...
    let(:order_id) { 123 }
    let(:stub_get_order_request) { stub_request(:get, endpoint_url_private + "/v1/user/spot/order?order_id=#{order_id}&pair=btc_jpy") }
  ...
  describe "#cancel_request!" do
    ...
    let(:stub_cancel_request) {
      stub_request(:post, endpoint_url_private + "/v1/user/spot/cancel_order")
    }
