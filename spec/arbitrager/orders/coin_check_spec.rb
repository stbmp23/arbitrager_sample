...
describe Arbitrager::Orders::CoinCheck do
  let(:order_ask) { Arbitrager::Orders::CoinCheck.create_ask(2_000_000.0, 0.001) }
  let(:order_bid) { Arbitrager::Orders::CoinCheck.create_bid(2_000_000.0, 0.001) }
      ...
      context "正常完了" do
        ...
        it "注文情報がモデルへ保存される" do
          ...
          expect(attributes).to eq({
            'broker_id' => Settings.broker.coincheck.id,
            'action_id' => Settings.actions.ask.id,
            'target_price' => 2_000_000.0,
            'target_volume' => 0.001,
            'order_acceptance_id' => '123',
            'result' => true,
            'response' => json,
  ...
  describe "#execution?" do
    ...
    let(:stub_get_order_request) { stub_request(:get, endpoint_url + "/api/exchange/orders/opens") }
    before { order_ask.model.order_acceptance_id = 12345 }
  ...
  describe "#cancel_request!" do
    let(:stub_cancel_request) {
      stub_request(:delete, endpoint_url + "/api/exchange/orders/#{order_acceptance_id}")
    }
