...
describe Arbitrager::Orders::BitFlyer do
  let(:order_ask) { Arbitrager::Orders::BitFlyer.create_ask(2_000_000.0, 0.001) }
  let(:order_bid) { Arbitrager::Orders::BitFlyer.create_bid(2_000_000.0, 0.001) }
      ...
      context "正常完了" do
        ...
        it "注文情報がモデルへ保存される" do
          ...
          expect(attributes).to eq({
            'broker_id' => Settings.broker.bitflyer.id,
            'action_id' => Settings.actions.ask.id,
            'target_price' => 2_000_000.0,
            'target_volume' => 0.001,
            'order_acceptance_id' => 'JRF20150707-050237-639234',
            'result' => true,
            'response' => json,
  ...
  describe "#execution?" do
    ...
    let(:stub_get_order_request) { stub_request(:get, endpoint_url + "/v1/me/getchildorders?" + params.to_query) }
    let(:params) {
      {
        order_acceptance_id: 'JRF20150707-084552-031927',
        product_code: 'BTC_JPY',
      }
    }
    before { order_ask.model.order_acceptance_id = params[:order_acceptance_id] }
    ...
    context "リクエストタイムアウト" do
      before { stub_get_order_request.to_timeout }
      it { is_expected.to be_falsey }
    end
  ...
  describe "#cancel_request!" do
    ...
    context "正常完了" do
      ...
      before {
        ...
        stub_cancel_request.to_return(:body => '', :status => 200)
      }
    ...
    context "リクエストタイムアウト" do
      ...
      before {
        order_ask.model.order_acceptance_id = '123'
        stub_cancel_request.to_timeout
      }
      it { is_expected.to be_falsey }
    end
  end
  ...
  describe "#update_execution" do
    ...
    let(:stub_get_orders) {
      stub_request(:get, endpoint_url + "/v1/me/getchildorders?product_code=BTC_JPY&order_acceptance_id=#{order_acceptance_id}")
    }
    let(:order_acceptance_id) { 'JRF20150707-084552-031927' }
    ...
    context "約定済みの注文が取得できなかった場合" do
      before {
        order_ask.model.order_acceptance_id = order_acceptance_id
        stub_get_orders.to_return(body: [].to_json, status: 200)
      }
