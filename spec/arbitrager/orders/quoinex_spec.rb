...
describe Arbitrager::Orders::Quoinex do
  let(:order_ask) { Arbitrager::Orders::Quoinex.create_ask(2_000_000.0, 0.001) }
  let(:order_bid) { Arbitrager::Orders::Quoinex.create_bid(2_000_000.0, 0.001) }
      ...
      context "正常完了" do
        ...
        it "注文情報がモデルへ保存される" do
          ...
          expect(attributes).to eq({
            'broker_id' => Settings.broker.quoinex.id,
            'action_id' => Settings.actions.ask.id,
            'target_price' => 2_000_000.0,
            'target_volume' => 0.001,
            'order_acceptance_id' => '2157474',
            'result' => true,
            'response' => json,
  ...
  describe "#execution?" do
    ...
    let(:stub_get_order_request) { stub_request(:get, endpoint_url + "/orders/#{order_id}") }
  ...
  describe "#cancel_request!" do
    ...
    let(:stub_cancel_request) {
      stub_request(:put, endpoint_url + "/orders/#{order_id}/cancel")
    }
